...
export abstract class Node<Config extends NodeConfig = NodeConfig> {
  ...
  toObject() {
    ...
    for (key in attrs) {
      ...
      // a custom attribute may share its name with a node method, so only
      // the accessors generated by Factory are asked for the default value
      getter = this[key]?._isAttrAccessor && this[key];
