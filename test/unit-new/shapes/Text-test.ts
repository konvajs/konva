...
describe('Text', function () {
  ...
  it('text cache with fill and shadow', function () {
    ...
    layer2.add(text1.clone().cache({ pixelRatio: 4 }));
  ...
  it('get text width of long text with spacing (check it visually!)', function () {
    ...
    if (isBrowser) {
      assert.almostEqual(Math.round(text1.width()), 1725);
      assert.almostEqual(Math.round(text2.width()), 2613);
      assert.almostEqual(Math.round(text3.width()), 2005);
      assert.almostEqual(Math.round(text4.width()), 1932);
    } else {
      assert.almostEqual(Math.round(text1.width()), 1745);
      assert.almostEqual(Math.round(text2.width()), 2633);
      assert.almostEqual(Math.round(text3.width()), 1995);
      assert.almostEqual(Math.round(text4.width()), 1981);
    }
  });
  ...
  it.skip('we should be able to clip underline by group', function () {
    var stage = addStage();
    var layer = new Konva.Layer();
    stage.add(layer);
